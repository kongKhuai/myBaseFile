...
## 描述
运行一个`Vue`实例并将`data`打印，可以看到对象中对于`msg`有了`get`与`set`，通过他们就可以实现数据的劫持，从而进行数据的更新，在`Vue`中`get`与`set`是通过`ES5`的`Object.defineProperty()`方法定义的，该方法的具体功能可以查阅` /blob/master/JavaScript/defineProperty.md`。
